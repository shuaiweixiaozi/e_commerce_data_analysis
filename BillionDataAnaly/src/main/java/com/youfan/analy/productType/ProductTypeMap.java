package com.youfan.analy.productType;
...
public class ProductTypeMap implements FlatMapFunction<String, String> {
    private final  static String productTypeTop = "productTypeTop";
    @Override
    public void flatMap(String s, Collector<String> collector) throws Exception {
        ...
        String productTypeid = HbaseUtil2.getdata("productinfo",productId+"","data","productTypeid");
         Integer status = orderinfo.getStatus();
        //private Integer status;// '0未支付 1 已支付 2 已退款 3 未支付已失效',
        if(status==1||status == 2){
            RedisUtils.incrScoreData(productTypeTop,1,productTypeid+"");
        }
